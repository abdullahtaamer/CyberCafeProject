...
void SaveCustomersToFile(const char* filename) {
    ...
    for (int i = 0; i < customers_num; ++i) {
        if (Custs[i].ID != 0) {
            out << Custs[i].ID << ' ' << Custs[i].name << ' ' << Custs[i].password << ' ' << Custs[i].res << '\n';
            for (int j = 0; j < Custs[i].res; ++j) {
                out << Custs[i].reservations[j].game << ' '
                    << Custs[i].reservations[j].type << ' '
                    << Custs[i].reservations[j].time << '\n';
            }
        }
    }
}
...
void LoadCustomersFromFile(const char* filename) {
    ...
    while (std::getline(in, line)) {
        ...
        if (!(headerStream >> id >> name >> pass >> rescount)) continue;

        int i = 0;
        while (i < customers_num && Custs[i].ID != 0) ++i;
        if (i >= customers_num) break;

        Custs[i].ID = id;
        Custs[i].name = name;
        Custs[i].password = pass;
        Custs[i].res = resCount;
        ...
        for (int j = 0; j < resCount; ++j) {
            ...
            int game, time;
            std::string typeStr;
            resStream >> game >> typeStr >> time;
            Custs[i].reservations[j].game = game;
            Custs[i].reservations[j].type = typeStr.empty() ? '\0' : typeStr[0];
            Custs[i].reservations[j].time = time;
        }
    }
}
